...
from bacpypes.apdu import WhoIsRequest, IAmRequest, ReadPropertyRequest, ReadPropertyMultipleRequest, ReadPropertyACK, \
...
def read_multiple_props(app, address, obj_type, obj_inst, prop_id, index=None):
    request = ReadPropertyMultipleRequest(
                ReadAccessSpecification(
                    objectIdentifier=(obj_type, obj_inst),
                    listOfPropertyReferences=[PropertyReference(propertyIdentifier=prop_id)]
                ))
...
def main():
    ...
    arg_parser.add_argument("--index", nargs='?', type=float,
                            help='Index of a point.',
                            default=None)
    ...
    print(read_multiple_props(this_application, target_address, "device", device_id, "all", index=index))
    # try:
    #     device_name = read_prop(grab_bacnet_config.py)
